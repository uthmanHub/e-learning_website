...
const page = () => {
  ...
  const currentTestimony = data.find(item => Number(item.id) === current);
  const forwardn = () => {
    setCurrent(prev => (prev < data.length ? prev + 1 : 1));
  };
    ...
    <div className='flex flex-col gap-[50px] w-full min-w-fit mt-10  md:flex-row-reverse items-center'>
      ...
      <div className='w-full py-[30px] px-3 md:px-10 bg-white rounded-md grid gap-10'>
        <div className='text-center '>
          <h2 className='font-semibold text-[28px] text-gray15 '>Login</h2>
        ...
        <div className='space-y-6 w-full min-w-fit '>
          ...
          <div className='space-y-6 w-full grid'>
            <hr></hr>
